...
def _create_xlDa_chunk_data(metadata):
    ...
    try:
        ...
        for ann_idx, ann in enumerate(annotations):
            ...
            # No else needed as _validate_metadata should have caught unknown formats
...
def add_xlabel_metadata_to_png(input_image_path, output_image_path, metadata, overwrite=False):
    ...
    if not overwrite and os.path.exists(output_image_path):
        ...
        logger.error(msg)
        raise FileExistsError(msg) # More specific than generic XLabelError
        
    try:
        # Validate metadata structure first
        _validate_metadata(metadata) # Can raise XLabelFormatError
    except XLabelFormatError as e:
        logger.error(f"Metadata validation failed: {e}")
        raise # Re-raise the specific format error

    try:
        img = Image.open(input_image_path)
        # Handle image mode conversion for PNG saving
        if img.mode not in ['RGB', 'RGBA', 'L', 'LA', 'P']:
            ...
            img = img.convert("RGBA")
             ...
             if 'transparency' in img.info: # Palette with transparency
        ...
        # Crucial: Update metadata with actual image dimensions from loaded image AND ensure correct XLABEL_VERSION
        # This ensures the metadata accurately reflects the image it's saved with and the format version.
        metadata["image_properties"]["width"] = img.width
        metadata["image_properties"]["height"] = img.height
        metadata["xlabel_version"] = XLABEL_VERSION # Set the version this creator is using

        chunk_data_bytes = _create_xlDa_chunk_data(metadata) # Can raise XLabelFormatError or XLabelError
        ...
        raise # Reraise to be explicit
    except XLabelError: # Catch our specific errors from _create_xlDa_chunk_data or _validate_metadata
        raise
    except IOError as e: # Catch PIL Image.open or Image.save errors
...
if __name__ == '__main__':
    ...
    test_metadata_v0_2_0 = {
        "xlabel_version": "0.2.0", # This will be overridden by XLABEL_VERSION in add_xlabel_metadata_to_png
        "image_properties": {"filename": "test_image_creator.jpg", "width": 320, "height": 240}, # width/height also overridden
    ...
    invalid_metadata_test = {"image_properties": {"filename":"test.png"}} # Missing width/height, class_names etc.
    ...
    try:
        ...
        img_dummy = Image.new('RGB', (320, 240), color = 'blue') # Use arbitrary size for dummy
        ...
        # Ensure output_png_with_xlabel exists from Test 1
        ...
        if os.path.exists(dummy_input_image):
            os.remove(dummy_input_image)
        if os.path.exists("invalid_output.png"): # cleanup
            os.remove("invalid_output.png")